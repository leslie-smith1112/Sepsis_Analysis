...
dat_batch_adjusted_norm_new <- ComBat(raw_disease, batch.disease, mod.mat)
to_write <- as.data.frame(dat_batch_adjusted_norm_new) %>% rownames_to_column("Gene") # sampels are logged wit no repeats
write.table(to_write, file = "/home/leslie.smith1/blue_kgraim/leslie.smith1/SEPSIS/05_20_23_disease_samples_Faheem_withmodmat_26batch.tsv", sep = "\t", col.names = TRUE, row.names = FALSE)
...
deseq_object2[deseq_object$Gene == "CYP51A1",]#FALSE
deseq_object2[deseq_object$Gene == "CYP4F2",] #TRUE

deseq_object2[deseq_object2$Gene == "LDLR",] #TRUE
deseq_object2[deseq_object2$Gene == "DHCR24",] #TRUE
deseq_object2[deseq_object2$Gene == "DHCR7",] #TRUE
deseq_object2[deseq_object2$Gene == "MSMO1",] #TRUE
deseq_object2[deseq_object2$Gene == "HMGCR",] #TRUE
deseq_object2[deseq_object2$Gene == "PCSK9",] #TRUE
