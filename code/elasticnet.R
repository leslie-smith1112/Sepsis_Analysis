...
dim(meta)
train <- meta %>% select(geo_accession, ARDs)
# get test samples
# mine<- c("ARDs")
# test <- meta.dat[meta.dat$ARDs %in% mine,]
# dim(test)
temp <- c("UF ARDs","UF ARDs")
train$ARDs[!(train$ARDs %in% temp)] <- "noARDS"
train$ARDs[train$ARDs %in% temp] <- "ARDs"
dim(train)
table(train$ARDs)
