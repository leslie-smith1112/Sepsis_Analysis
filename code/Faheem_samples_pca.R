...
#sample names have directory included to we must cut each sample name to make it neat. Done independently for each run.
...
write.table(dat,here("datasets", "Faheem","expression.tsv"), sep = "\t", col.names = TRUE,row.names=FALSE)
...
new.dat[1:5,1:5]
write.table(new.dat,here("datasets", "Faheem","expression_logged.tsv"), sep = "\t", col.names = TRUE,row.names=FALSE)
...
# write to file
to_write <- as.data.frame(f.batch.ids)
to_write <- rownames_to_column(to_write,"sample")
head(to_write)
colnames(to_write) <- c("sample","run")
write.table(to_write,here("datasets", "Faheem","batch_ids.tsv"), sep = "\t", col.names = TRUE,row.names=FALSE)
